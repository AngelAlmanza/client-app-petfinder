  ...
  return (
    <MainLayout>
      <section className="w-full h-72 relative">
        ...
        <span
          className="w-3/4 inline-flex justify-between items-center rounded-2xl py-1 px-3
          bg-red-500 font-bold text-white uppercase text-xl absolute bottom-0
            left-1/2 -translate-x-1/2 translate-y-1/2"
        >
          <FontAwesomeIcon icon={faPaw} size="2x" className="text-white text-xs " />
          dante
          <FontAwesomeIcon icon={faPaw} size="2x" className="text-white text-xs " />
        </span>
      </section>

      <section className="w-100 relative flex gap-2 items-center mt-8">
        <div className="w-3/4 flex items-center gap-4">
          <div className="w-12 h-12 rounded-full overflow-hidden shadow-xl filter">
            <img className="w-full h-full" src="https://via.placeholder.com/150" />
          </div>
          <div>
            <p className="text-lg line-clamp-1 text-primary-color font-bold">Nombre de Usuario</p>
            <p className="text-sm line-clamp-1 text-text-primary-color font-medium">Nombre de la ciudad</p>
          </div>
        </div>
      ...
      <section>
        <h4 className="my-4 text-primary-color font-bold text-lg">Etiquetas</h4>
      ...
      <section>
        <h4 className="my-4 text-primary-color font-bold text-lg">Visto por última vez en:</h4>
      ...
      <section>
        <h4 className="my-4 text-primary-color font-bold text-lg">Detalles</h4>
        <p className="text-sm text-text-primary-color font-medium">
          Descripción de los detalles que proporcionó el usuario al crear la publicación.
        </p>
      </section>

      <section className="flex items-center justify-evenly mb-4 mt-8">
        <button>
          <FontAwesomeIcon icon={faArrowLeft} className="text-sm text-text-primary-color font-semibold" />
          <span className="ml-2 text-sm text-text-primary-color font-semibold underline">Volver</span>
        </button>
        <button className="bg-ligth-green flex justify-evenly items-center px-3 py-2 rounded-2xl shadow-lg">
          <FontAwesomeIcon icon={faPhone} className="text-sm text-white font-semibold" />
          <span className="ml-2 text-sm text-white font-semibold">Contactar</span>
        </button>
      </section>
    </MainLayout>
  )
